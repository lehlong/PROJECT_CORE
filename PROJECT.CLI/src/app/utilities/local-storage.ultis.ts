...
const SECRET_KEY = environment.SECRET_KEY_LOCAL_STORAGE;

export class LocalStorageUtils {
    static setItem(key: string, value: any): void {
        try {
            const jsonData = JSON.stringify(value);
            const encrypted = AES.encrypt(jsonData, SECRET_KEY).toString();
    ...
    static debugDecrypt(key: string): string | null {
        try {
            ...
            return AES.decrypt(encrypted, SECRET_KEY).toString(Utf8);
        } catch (error) {
